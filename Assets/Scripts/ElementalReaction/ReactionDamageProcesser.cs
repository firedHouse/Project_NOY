using UnityEngine;

public static class ReactionDamageProcesser
{//
 //   public static void Apply(ElementReaction reaction, Unit hitTarget, Unit[] enemyTeam)
 //   {
 //       switch (reaction)
 //       {
 //           case ElementReaction.Vaporize:
 //               ApplyVaporize(hitTarget);
 //               Debug.Log("증발 발생");
 //               break;
 //           case ElementReaction.ElectroShock:
 //               ApplyElectroShock(hitTarget, enemyTeam);
 //               Debug.Log("감전 발생");
 //               break;
 //           case ElementReaction.Overload:
 //               ApplyOverload(hitTarget, enemyTeam);
 //               Debug.Log("과부하 발생");
 //               break;
 //       }
 //   }
 //   //각 원소 발생 데미지 메서드들 -> 과부하는 턴마다 데미지이나, 이는 다른 곳에서 처리해볼 생각이다.
 //   private static void ApplyVaporize(Unit target)
 //   {
 //       float damage = target.MaxHP * 0.1f;
 //       target.TakeDamage(damage);
 //   }
 //
 //   private static void ApplyElectroShock(Unit target, Unit[] enemyTeam)
 //   {
 //       float damage = target.MaxHP * 0.07f;
 //
 //       foreach (var enemy in enemyTeam)
 //       {
 //           if (enemy != target && !enemy.IsDead)
 //           {
 //               enemy.TakeDamage(damage);
 //           }
 //       }
 //   }
 //
 //   private static void ApplyOverload(Unit target, Unit[] enemyTeam)
 //   {
 //       float damage = target.MaxHP * 0.04f;
 //       foreach (var enemy in enemyTeam)
 //       {
 //           if (enemy != target && !enemy.IsDead)
 //           {
 //               enemy.TakeDamage(damage);
 //           }
 //       }
 //   }
}
